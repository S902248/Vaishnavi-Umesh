...
const Petal = ({ id }) => {
  ...
  useEffect(() => {
    ...
    setStyle({
      ...
      boxShadow: isMobile ? 'none' : '0 0 10px rgba(212, 175, 55, 0.1)',
...
const Firefly = ({ id }) => {
  ...
  return (
    <div 
      className={`absolute w-1 h-1 bg-royal-gold rounded-full opacity-40 ${isMobile ? '' : 'blur-[1px]'}`} 
      style={style} 
    />
  );
};
    ...
    <div className="absolute" style={{ left: `${position.x}%`, top: `${position.y}%` }}>
      {Array.from({ length: isMobile ? 6 : 15 }).map((_, i) => (
        <motion.div
          ...
          style={isMobile ? {} : { boxShadow: '0 0 10px #d4af37' }}
        />
      ))}
    </div>
  );
...
export default function Decorations() {
  ...
  useEffect(() => {
    setItems(Array.from({ length: isMobile ? 12 : 40 }, (_, i) => i));
    const interval = setInterval(() => {
      if (Math.random() > (isMobile ? 0.9 : 0.6)) {
        const id = Date.now();
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-20 overflow-hidden">
      {/* Petals */}
      {items.slice(0, isMobile ? 4 : 15).map(id => <Petal key={`petal-${id}`} />)}
      
      {/* Fireflies */}
      {items.slice(0, isMobile ? 8 : 25).map(id => <Firefly key={`firefly-${id}`} />)}
      ...
      {/* Bokeh Royal Orbs - Simplified on mobile */}
      {!isMobile && (
        <>
